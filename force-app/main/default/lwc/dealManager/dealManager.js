...
export default class DealManager extends LightningElement {
    ...
    handleRowAction(event) {
        ...
        if (action === 'new_request') {
            ...
            // simplistic stage change
            const newStage = prompt('Enter new stage', row.stage);
            if (newStage) {
                updateDealStage({ opportunityId: row.opportunityId, newStage })
                    .then(() => {
                        this.stageFilter = this.stageFilter; // refresh via wire
                    })
                    .catch(error => {
                        // eslint-disable-next-line no-console
                        console.error(error);
    ...
    handleCaseAction(event) {
        ...
        if (action === 'update_status') {
            const newStatus = prompt('Enter new status', row.Status);
            if (newStatus) {
                updateCaseStatus({ caseId: row.Id, newStatus })
                    .then(() => {
                        this.stageFilter = this.stageFilter; // refresh data
                    })
                    .catch(error => {
                        // eslint-disable-next-line no-console
                        console.error(error);
